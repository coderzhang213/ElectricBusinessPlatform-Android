...
import com.sancell.xingqiu.utils.FansUtils
import com.sancell.xingqiu.utils.MyScreenUtils
import com.sancell.xingqiu.utils.TimeUtil

/**
 * 首页推荐列表
 */
class HomeLiveVideoAdapter(data: List<LiveFollowInfo>) : BaseQuickAdapter<LiveFollowInfo, BaseViewHolder>(
    R.layout.recycle_home_live_video_living, data) {
    ...
    override fun convert(helper: BaseViewHolder, item: LiveFollowInfo) {
        ...
        if (item.relationGoodsList.isNullOrEmpty()) {
            ...
            //数据测量出来后
            iv_video_thumb.post {
                mGoodsAdapter.setViewWidth(iv_video_thumb.width - MyScreenUtils.dip2px(mContext, 17.5f))//减去设计图的margin
                        .setItemMargin(MyScreenUtils.dip2px(mContext, 12f))
...
//        }
        ...
        when (item.liveStatus) {
            "1" -> {
                ...
                tv_looking_num.text = TimeUtil.getDateFromMMDDHH(item.reserveStartLiveTime)
            ...
            "2" -> {
                ...
                //修改图标尺寸
                val params = iv_live_status.layoutParams
                params.width = MyScreenUtils.dip2px(mContext, 18f)
                params.height = params.width
                iv_live_status.layoutParams = params
