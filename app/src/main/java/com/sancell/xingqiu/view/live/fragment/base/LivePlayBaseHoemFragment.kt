...
import com.sancell.xingqiu.view.live.fragment.LivePlayListFragment2
import kotlinx.android.synthetic.main.fragment_play_base_layouy.*

class LivePlayBaseHoemFragment : BaseDataFragmentKt<LiveViewModel>(), OnAddRecommendLinsener {
    ...
    companion object {
        fun getInsener(mType: String, batchId: String, anchorId: String): Fragment {//不要左右切换了，所以直接替换
            ...
            if (mType == LivePlayType.SERARCH_TYPE.type ||
                ...
                framgent = LivePlayBaseFragment.getInstet(mType, batchId, 0, object :
                    OnAddRecommendLinsener {
                    override fun onAddRecommendData(dataList: List<RecomLiveInfo>?) {
                    }
                })
            } else {
                framgent =
                    LivePlayListFragment2.startIntent(
                        mType,
                        batchId,
                        anchorId
                    )
