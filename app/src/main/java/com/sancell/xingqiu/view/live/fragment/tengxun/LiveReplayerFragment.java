package com.sancell.xingqiu.view.live.fragment.tengxun;
...
import com.sancell.xingqiu.entity.live.FloatLiveInfo;
...
import com.sancell.xingqiu.interfaces.OnGetCurrPagerIsShowLinsener;
import com.sancell.xingqiu.interfaces.OnLivePlayManager;
import com.sancell.xingqiu.interfaces.OnLiveStatusLinsener;
import com.sancell.xingqiu.interfaces.OnLiveToolLinsener;
import com.sancell.xingqiu.services.FloatWindowService;
...
import cn.sancell.xingqiu.live.base.LiveReplayToolBaseFragment;
 ...
 */
public class LiveReplayerFragment extends LiveReplayToolBaseFragment implements OnClickListener
        , OnLiveToolLinsener, OnLiveStatusLinsener,
        OnGetCurrPagerIsShowLinsener {
    private static final String TAG = LiveReplayerFragment.class.getSimpleName();
    ...
    private void initPar() {
        ...
        roomId = arguments.getString(LiveConstant.EXTRA_ROOM_ID);
    ...
    public int getLayoutId() {
        return R.layout.fragment_tengxun_replay;
    }
...
//        }).preload();
    ...
    public void showFloat() {
        super.showFloat();
        FloatLiveInfo mFloatLiveInfo = new FloatLiveInfo();
        mFloatLiveInfo.liveType = mActivityType;
        mFloatLiveInfo.playUrl = mVideoPath;
        mFloatLiveInfo.isLivePlay = isLivePlay;
        if (ServerUtils.INSTANCE.isServiceRunning(getActivity(), FloatWindowService.class.getName())) {
            ObserverManger.getInstance(ObserverKey.LIVE_FLOATE_PLAY).notifyObserver(mFloatLiveInfo);
        } else {
            //启动服务并结束
            ServerUtils.INSTANCE.startFloatServer(getActivity(), mFloatLiveInfo);

        }
    }

    @Override
    public void onToolShowCoupon() {
        setShowCoupon();
    }

    @NotNull
    @Override
    public LivePlayInfo getToolLiveInfoData() {
        return getLiveInfoData();
    }

    @Override
    public void onToolPagerCloce() {
        onPagerCloce();
    }

    @Override
    public void onToolUpLikeSum(int sum) {
        onUpLikeSum(sum);
    }

    @Override
    public void onToolUpLiveWahtSum(int sum) {
        onUpLiveWahtSum(sum);
    }

    @Override
    public void onToolSendMsgIsShow(boolean isShow) {
