...
import com.sancell.xingqiu.view.live.fragment.LiveRoomFragment
import com.sancell.xingqiu.view.live.fragment.LiveSubscribeFragment
import kotlinx.android.synthetic.main.fragment_live_palyt_base_layout.*

/**
 *用来区分加载什么播放类型
 */
class LivePlayBaseFragment : BaseDataFragmentKt<LiveViewModel>(), OnBackPressedLinsener {
    ...
    fun loadFragment(mInfo: LivePlayInfo?) {
        mInfo?.apply {
            ...
            if (this.batchInfo.liveStatus == "1") {//未开始
                fragment =
                    LiveSubscribeFragment.startIntent(
                        batchId!!,
                        postion,
                        mType!!
                    )
