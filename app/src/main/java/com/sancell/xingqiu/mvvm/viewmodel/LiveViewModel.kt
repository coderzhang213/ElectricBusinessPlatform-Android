...
import com.sancell.xingqiu.mvvm.repository.BannerRepository
import com.sancell.xingqiu.mvvm.repository.LiveRepository
import com.sancell.xingqiu.mvvm.repository.LiveUserRepository
import com.sancell.xingqiu.mvvm.repository.VoucherRepository
...
class LiveViewModel : BaseViewModel() {
    ...
    fun upRoomStatus(status: String): MutableLiveData<LiveRoomUpInfo> {
        ...
        par["accid"] = UserManager.getUserSkey()
        launch {
            val response = withContext(Dispatchers.IO) {
    ...
    fun getLiveSums(type: String, roomId: String, batchId: String): MutableLiveData<LiveCountInfo> {
        val reqTime = StringCheckUtils.getCurrentTime()
        val skey = UserManager.getUserSkey()
    ...
    fun getBannerData(bannerAdType: String) {
        ...
        par["reqTime"] = StringCheckUtils.getCurrentTime()
        par["skey"] = UserManager.getUserSkey()
        par["hashToken"] = RSAUtils.encryptByPublic(par)
        launch {
            val response = withContext(Dispatchers.IO) {
